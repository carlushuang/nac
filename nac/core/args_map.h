...
template<>
inline int args_map::insert<const std::string>(std::string arg_name, const std::string value){
    if(arg_map_.find(arg_name) != arg_map_.end()){
        NAC_WARNING("try to insert duplicated arg:", arg_name, " with value:",
...
template<>
inline int args_map::insert<const char*>(std::string arg_name, const char* value){
    if(arg_map_.find(arg_name) != arg_map_.end()){
        NAC_WARNING("try to insert duplicated arg:", arg_name, " with value:",
            value, ", origin value is:", arg_map_[arg_name]);
    }
